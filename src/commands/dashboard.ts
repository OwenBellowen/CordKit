...
function analyzeProject(projectPath: string): ProjectStats {
  ...
  const isTypeScript = pkg.main?.endsWith('.ts') || 
  ...
  if (pkg.dependencies?.sqlite3 || pkg.dependencies?.sqlite) features.push('database');
  ...
  if (existsSync(join(projectPath, 'slash-commands'))) features.push('slash-commands');
  ...
  let botType = 'general';
  if (pkg.keywords?.includes('music') || features.includes('music')) botType = 'music';
  else if (features.includes('automod') || pkg.dependencies?.['bad-words']) botType = 'moderation';
  else if (features.includes('economy') || features.includes('levels')) botType = 'utility';
  
  // Count files and lines
  const { fileCount, totalLines } = countProjectFiles(projectPath, isTypeScript ? '.ts' : '.js');
...
function countProjectFiles(projectPath: string, extension: string): { fileCount: number; totalLines: number } {
  let fileCount = 0;
  let totalLines = 0;
  
  const searchDirectories = ['', 'commands', 'slash-commands', 'utils', 'plugins', 'database', 'webhooks'];
...
function displayDashboard(stats: ProjectStats, detailed: boolean) {
  ...
  // Basic Information
  console.log(`🤖 ${chalk.cyan('Project:')} ${chalk.white.bold(stats.name)} ${chalk.gray('v' + stats.version)}`);
  console.log(`📝 ${chalk.cyan('Template:')} ${stats.template === 'typescript' ? chalk.blue('📘 TypeScript') : chalk.yellow('📙 JavaScript')}`);
  console.log(`🎯 ${chalk.cyan('Bot Type:')} ${getBotTypeEmoji(stats.botType)} ${chalk.green(stats.botType)}`);
  console.log(`📅 ${chalk.cyan('Last Modified:')} ${chalk.white(stats.lastModified.toLocaleDateString())}`);
  console.log('');
  
  // Code Statistics
  console.log(chalk.yellow.bold(`📈 Code Statistics:`));
  console.log(`   ${chalk.cyan('Files:')} ${chalk.white.bold(stats.fileCount)}`);
  console.log(`   ${chalk.cyan('Lines of Code:')} ${chalk.white.bold(stats.totalLines.toLocaleString())}`);
  console.log('');
  ...
  if (stats.features.length > 0) {
    ...
    for (let i = 0; i < stats.features.length; i += 3) {
      const row = stats.features.slice(i, i + 3).map(f => getFeatureEmoji(f) + chalk.green(f)).join('  ');
  ...
  console.log(`📦 ${chalk.cyan('Dependencies:')} ${chalk.white.bold(depCount)} production, ${chalk.white.bold(devDepCount)} development`);
  ...
  if (detailed) {
    ...
    const keyDeps = ['discord.js', 'dotenv', 'winston', 'sqlite3', '@discordjs/voice'];
    keyDeps.forEach(dep => {
      if (stats.dependencies[dep]) {
        console.log(`   ${chalk.cyan(dep)}: ${chalk.white(stats.dependencies[dep])}`);
      }
    });
    ...
    devTools.forEach(tool => {
      if (stats.devDependencies[tool]) {
        console.log(`   ${chalk.cyan(tool)}: ${chalk.white(stats.devDependencies[tool])}`);
      }
    });
...
function checkProjectHealth(stats: ProjectStats): Array<{ status: string; message: string }> {
  const checks = [];
  ...
  if (stats.dependencies['discord.js']) {
    ...
    if (version.includes('14.')) {
      ...
      checks.push({ status: '⚠️', message: 'Discord.js version may be outdated' });
    }
  }
  ...
  if (stats.template === 'typescript') {
    if (stats.devDependencies.typescript) {
      ...
      checks.push({ status: '❌', message: 'TypeScript missing from devDependencies' });
    }
  }
  ...
  if (stats.fileCount > 20) {
    checks.push({ status: '✅', message: 'Well-structured project with multiple files' });
...
function generateRecommendations(stats: ProjectStats): string[] {
  ...
  if (!stats.features.includes('dotenv')) {
    recommendations.push('Use dotenv for secure environment variable management');
  }
  ...
  if (recommendations.length === 0) {
    recommendations.push('Your project looks great! Consider adding more features with plugins');
  }

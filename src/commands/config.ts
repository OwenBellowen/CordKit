...
export const configCommand = new Command("config")
  ...
  .action((opts: any) => {
    ...
    if (!existsSync(configPath)) {
      console.error(
        chalk.red("❌ No config.json found. Use --init to create one."),
      );
      process.exit(1);
    }
    ...
    console.log(
      chalk.white("  --init              ") +
        chalk.gray("Initialize configuration file"),
    );
    console.log(
      chalk.white("  --get <key>         ") +
    ...
    console.log(
      chalk.green("\nExample: ") +
        chalk.yellow('cordkit config --set prefix "?"'),
    );
  });
...
function initConfig(configPath: string) {
  ...
  console.log(
    chalk.yellow('💡 Use "cordkit config --list" to see all options'),
  );
}
...
function listConfig(configPath: string) {
  try {
    ...
    console.log("\n🔒 Permissions:");
    console.log(
      `  requireRoles: ${config.permissions.requireRoles ? "✅" : "❌"}`,
    );
    console.log(
      `  allowedChannels: ${config.permissions.allowedChannels.length} channels`,
    );
  } catch (error) {

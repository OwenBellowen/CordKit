  ...
  .action(async (options) => {
    try {
      ...
      if (!existsSync(join(projectPath, "package.json"))) {
        console.log(
          chalk.red(
            "❌ No package.json found. Make sure you're in a CordKit project directory.",
          ),
        );
        return;
      }
...
async function promptGenerateOptions(cliOpts: any): Promise<GenerateOptions> {
  ...
  const additionalAnswers = additionalQuestions.length
    ? await prompts(additionalQuestions)
    : {};
...
async function generateComponent(
  ...
  if (options.type === "command") {
    console.log(
      chalk.yellow(
        "   • Import and register the command in your main bot file",
      ),
    );
    ...
    console.log(
      chalk.yellow("   • Import and register the event in your main bot file"),
    );
  }
}

async function generateMessageCommand(
  options: GenerateOptions,
  projectPath: string,
  ext: string,
) {
...
async function generateSlashCommand(
  options: GenerateOptions,
  projectPath: string,
  ext: string,
) {
...
async function generateEventHandler(
  options: GenerateOptions,
  projectPath: string,
  ext: string,
) {
  ...
  const content =
    ext === "ts"
      ? generateEventHandlerTS(options)
      : generateEventHandlerJS(options);
...
export const data = new SlashCommandBuilder()
  ...
  .setDescription('${options.description || `A ${options.name} command`}')${
    options.permissions && options.permissions.length > 0
      ? `
